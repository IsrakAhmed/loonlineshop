...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        try {
            ...
            $menucategories = Category::where(['front_view' => 1, 'status' => 1])->select('id', 'name', 'slug', 'status', 'image')->get();
            ...
            // Handle database connection error seamlessly
            // Maybe log it or just let the app continue with empty values if that's safer, 
            // but for now, we just want to avoid the white screen of death if possible, 
            // or at least make it clear. 
            // However, critical data like 'generalsetting' is needed for views.
            // So if this fails, the app is effectively broken anyway.
            // But the error explicitly said 'target machine actively refused', so MySQL is OFF.

            // To prevent total crash during migration or artisan commands:
            return;
        }
    }
}
