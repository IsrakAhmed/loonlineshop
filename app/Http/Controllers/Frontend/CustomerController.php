...
class CustomerController extends Controller
{
    ...
    public function order_save(Request $request)
    {
        ...
        $order->user_ip = $request->ip(); // IP স্টোর হচ্ছে
        ...
        try {
            \Log::info('Attempting to send email for order #' . $order->invoice_id);

            if (config('mail.admin.address')) {
                FacadesMail::to(config('mail.admin.address'))
                    ->send(new AdminOrderNotification($order));

                \Log::info('Email sent successfully for order #' . $order->invoice_id);
            } else {
                \Log::error('Admin email address not configured');
            }
        } catch (\Exception $e) {
            \Log::error('Email sending failed for order #' . $order->invoice_id . ': ' . $e->getMessage());
        }
