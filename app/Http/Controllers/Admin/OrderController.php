...
class OrderController extends Controller
{
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $show_data = Order::latest()->with('orderdetails', 'shipping', 'status');
            if ($request->keyword) {
        ...
        return view('backEnd.order.index', compact('show_data', 'order_status', 'users', 'steadfast', 'pathaostore', 'pathaocities', 'activeApi'));
    }
    ...
    {
        $data = Order::where(['invoice_id' => $invoice_id])->select('id', 'invoice_id', 'order_status', 'admin_note')->with('orderdetails')->first();
    ...
    public function order_process(Request $request)
    {
        ...
        $shipping_update->additional_phone = $request->additional_phone;
    ...
    public function bulk_courier($slug, Request $request)
    {
        ...
        foreach ($orders_id as $order_id) {
            ...
            if (!$order || $order->order_status == 5) {
                $failed[] = "Order ID {$order_id} skipped (already couriered or not found).";
                continue;
            }
            ...
            $consignmentData = [
                'invoice' => $order->invoice_id,
                'recipient_name' => optional($order->shipping)->name ?? 'InboxHat',
                'recipient_phone' => optional($order->shipping)->phone ?? '01750578495',
                'recipient_address' => optional($order->shipping)->address ?? 'N/A',
                'cod_amount' => $order->amount,
            ...
            try {
                $response = Http::withHeaders([
                    ...
                    'Content-Type' => 'application/json',
                    'Accept' => 'application/json',
                ])->post($courier_info->url . '/create_order', $consignmentData);

                $res = $response->json();

                if (isset($res['status']) && $res['status'] == 200) {
                    $order->order_status = 4; // Placed to courier
                    $order->save();
                    $success[] = "Order {$order->invoice_id} placed successfully. Tracking: " . ($res['consignment']['tracking_code'] ?? 'N/A');
                } else {
                    $errorMessage = $res['message'] ?? 'Unknown error';
                    $failed[] = "Order {$order->invoice_id} failed: {$errorMessage}";
                }

            } catch (\Exception $e) {
                $failed[] = "Order {$order->invoice_id} exception: " . $e->getMessage();
            }
        }
    ...
    public function order_store(Request $request)
    {
        ...
        $shipping->additional_phone = $request->additional_phone;
    ...
    public function order_update(Request $request)
    {
        ...
        // STEP 1: আগের OrderDetails ID গুলো নিয়ে রাখা হচ্ছে
        $previousDetails = OrderDetails::where('order_id', $order->id)->pluck('id')->toArray();
        $currentDetails = []; // কার্টে যেসব আছে, সেগুলো এখানে রাখা হবে

        // STEP 2: কার্ট থেকে প্রোডাক্ট লুপ করে আপডেট / ইনসার্ট + ID সংগ্রহ
        foreach (Cart::instance('pos_shopping')->content() as $cart) {
            $detailsId = $cart->options->details_id ?? null;

            if ($detailsId) {
                $currentDetails[] = $detailsId; // কার্টে থাকা ID গুলো স্টোর হচ্ছে
            }

            $exits = OrderDetails::where('id', $detailsId)->first();
            ...
            if ($exits) {
                ...
                // নতুন ইনসার্ট হলে এর ID অ্যারে তে যোগ করবো
                $currentDetails[] = $order_details->id;
            }
        }

        // STEP 3: কার্টে না থাকা (ডিলিট হওয়া) প্রোডাক্টগুলোর ID বের করে ডিলিট
        $toDelete = array_diff($previousDetails, $currentDetails);
        if (!empty($toDelete)) {
            OrderDetails::whereIn('id', $toDelete)->delete();
        }
