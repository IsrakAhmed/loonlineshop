...
class CampaignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Manually assign each field to the Campaign model to avoid array-to-string conversion issues
        $campaign = new Campaign();
        $campaign->short_description = $request->input('short_description');
        $campaign->description = $request->input('description');
        $campaign->name = $request->input('name');
        $campaign->status = $request->input('status');
        $campaign->product_id = $request->input('product_id');
        $campaign->review = $request->input('review');
        ...
        if ($image1) {
            ...
            $width1 = $img1->width() > $img1->height() ? 800 : null;
            $height1 = $img1->width() > $img1->height() ? null : 600;
            ...
            $img1->save($image1Url);
            $campaign->image_one = $image1Url; // Save image path as string
        }
        ...
        if ($image2) {
            ...
            $width2 = $img2->width() > $img2->height() ? 800 : null;
            $height2 = $img2->width() > $img2->height() ? null : 600;
            ...
            $img2->save($image2Url);
            $campaign->image_two = $image2Url; // Save image path as string
        }
        ...
        if ($image3) {
            ...
            $width3 = $img3->width() > $img3->height() ? 800 : null;
            $height3 = $img3->width() > $img3->height() ? null : 600;
            ...
            $img3->save($image3Url);
            $campaign->image_three = $image3Url; // Save image path as string
        }
    
        // Create slug
        $campaign->slug = strtolower(Str::slug($request->name));
    
        // Save Campaign to the database
        $campaign->save();
    
        // Handle Multiple Images (Array to String Conversion Fix)
        ...
        if ($images) {
            foreach ($images as $image) {
                ...
                // Save the additional images into CampaignReview model
                $pimage = new CampaignReview();
                $pimage->campaign_id = $campaign->id;
                $pimage->image = $imageUrl; // Save image path as string
                $pimage->save();
            }
        }
